...
class CFWD(Recommender):

    NAME = "CFWD"

    def __init__(self, urm, ICM, S_matrix_target):
        self.urm = urm 

        if (urm.shape[1] != ICM.shape[0]):
            raise ValueError("Number of items not consistent. URM contains {} but ICM contains {}".format(urm.shape[1],
                                                                                                          ICM.shape[0]))
        ...
        self.S_matrix_target = self._check_matrix(S_matrix_target, 'csr')
        self.ICM = self._check_matrix(ICM, 'csr')
    ...
    def _generateTrainData_low_ram(self):
        ...
        self._writeLog(self.NAME + ": Collaborative S density: {:.2E}, nonzero cells {}".format(
            self.S_matrix_target.nnz/self.S_matrix_target.shape[0]**2, self.S_matrix_target.nnz))

        self._writeLog(self.NAME + ": Content S density: {:.2E}, nonzero cells {}".format(
        ...
        for row_index in range(self.n_items):
            ...
            if time.time() - start_time_batch > 30 or num_samples == S_matrix_contentKNN.nnz*(1+self.add_zeros_quota):
                ...
                print(self.NAME + ": Generating train data. Sample {} ( {:.2f} %) ".format(
        ...
        self._writeLog(self.NAME + ": Content S structure has {} out of {} ( {:.2f}%) nonzero collaborative cells".format(
        ...
        self._writeLog(self.NAME + ": Nonzero collaborative cell sum is: {:.2E}, average is: {:.2E}, "
    ...
    def fit(self, show_max_performance = False, logFile = None, loss_tolerance = 1e-6,
        ...
        self.r_hat = self.urm.dot(self.W_sparse)
